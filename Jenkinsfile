      ...
      stages{
        stage('Source Checkout'){
          steps {
            ...
            url: 'http://10.99.240.127:3000/FN-COVOL/Eureka.git'
            sh "ls -lat"
          }
        }
      stage('Build'){
      ...
      stage('Package'){
        steps{
          ...
          script {
                ...
                sh 'docker build -t ms-sat-covol-eureka-server:v1.0 . --tls-verify=false'  
        ...
        stage('Deploy'){
            steps{
                ...
                script {
                        sh 'docker tag ms-sat-covol-eureka-server:v1.0 docker-registry-default.cnh.dev.cloudb.sat.gob.mx/covol/ms-sat-covol-eureka-server:v1.0'
                        sh 'docker push docker-registry-default.cnh.dev.cloudb.sat.gob.mx/covol/ms-sat-covol-eureka-server:v1.0'
